...
export function buildStylesCss(input: CreativeTemplateInput): string {
  ...
  if (hasVideo && layout.videoSlot) {
    ...
    lines.push(
      ...
      '  display: none;',
      '}',
      '#creative.creative-failed > *:not(#click_area) {',
      '  display: none;',
      '}',
      '#creative.creative-failed {',
      '  background: transparent;',
      '}',
      '#video-controls {',
      '  position: absolute;',
      `  left: ${px(controlsLeft)};`,
      `  top: ${px(controlsTop)};`,
...
export function buildMainJs(): string {
  return [
    ...
    '  var controlPlay = document.getElementById("video-control-play");',
    '  var controlAudio = document.getElementById("video-control-audio");',
    '  var iconPlay = document.getElementById("icon-play");',
    '  var iconPause = document.getElementById("icon-pause");',
    '  var iconReplay = document.getElementById("icon-replay");',
    '  var iconAudioMuted = document.getElementById("icon-audio-muted");',
    '  var iconAudioOn = document.getElementById("icon-audio-on");',
    '',
    ...
    '    if (!video || !controlPlay) return;',
    '    var isPlaying = video.paused !== true && video.ended !== true;',
    '    setHidden(iconReplay, true);',
    '    setHidden(iconPause, !isPlaying);',
    '    setHidden(iconPlay, isPlaying);',
    '    controlPlay.setAttribute("aria-label", isPlaying ? "Pause video" : "Play video");',
    '  }',
    '',
    '  function syncAudioControl() {',
    '    if (!video || !controlAudio) return;',
    '    var muted = video.muted === true || video.volume === 0;',
    '    setHidden(iconAudioMuted, !muted);',
    '    setHidden(iconAudioOn, muted);',
    '    controlAudio.setAttribute("aria-label", muted ? "Unmute video" : "Mute video");',
    '  }',
    '',
    '  function safePlay() {',
    '    if (!video) return;',
    ...
    '  }',
    '',
    '  if (video && controlAudio) {',
    '    controlAudio.addEventListener("click", function (event) {',
    '      stopEvent(event);',
    '      var isMuted = video.muted === true || video.volume === 0;',
    '      if (isMuted) {',
    '        video.muted = false;',
    '        if (video.volume === 0) {',
    '          video.volume = 1;',
    ...
    '    video.addEventListener("playing", syncPlaybackControl);',
    ...
    '    var fallback = function () {',
      '      if (creative) {',
      '        creative.classList.add("creative-failed");',
      '      }',
    '      if (videoControls) {',
